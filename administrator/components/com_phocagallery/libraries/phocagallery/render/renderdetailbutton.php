...
class PhocaGalleryRenderDetailButton
{
	...
	function getNext ($catid, $id, $ordering)  {
		...
		foreach ($nextAll as $key => $value) {
			...
			if (isset($value->id) && $value->id > 0) {
				...
				.' title="'.JText::_( 'COM_PHOCAGALLERY_NEXT_IMAGE' ).'" id="next" onclick="disableBackAndNext()" >'
	...
	*/
	function getPrevious ($catid, $id, $ordering) {
		...
		foreach ($prevAll as $key => $value) {
			...
			if (isset($value->id) && $value->id > 0) {
				...
				.'<a href="'.JRoute::_('index.php?option=com_phocagallery&view=detail&catid='. $value->catslug.'&id='.$value->slug.$tmplCom.'&Itemid='. JRequest::getVar('Itemid', 1, 'get', 'int')).'"'
				.' title="'.JText::_( 'COM_PHOCAGALLERY_PREV_IMAGE' ).'" id="prev" onclick="disableBackAndPrev()" >'
	...
	function getCloseText($catidSlug, $idSlug) {
		...
		$close =  '<a style="text-decoration:underline" href="'.JRoute::_('index.php?option=com_phocagallery&view=detail&catid='.$catidSlug.'&id='.$idSlug.'&tmpl=component'.'&Itemid='. JRequest::getVar('Itemid', 1, 'get', 'int')).'" onclick="%onclickclose%" title="'.JText::_( 'COM_PHOCAGALLERY_CLOSE_WINDOW').'" >'. JText::_( 'COM_PHOCAGALLERY_CLOSE_WINDOW' ).'</a>';
