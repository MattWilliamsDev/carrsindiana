...
class PhocaGalleryCpControllerPhocaGalleryc extends JControllerForm
{
	...
	protected function allowAdd($data = array())
	{
		...
		$categoryId	= JArrayHelper::getValue($data, 'parent_id', JRequest::getInt('filter_category_id'), 'int');
		$allow		= null;

		if ($categoryId)
		{
			// If the category has been passed in the URL check it.
			$allow	= $user->authorise('core.create', 'com_phocagallery.phocagalleryc.'.$categoryId);
		}
		if ($allow === null)
		{
	...
	{
		// Initialise variables.
		$categoryId	= (int) isset($data['parent_id']) ? $data['parent_id'] : 0;
		$user		= JFactory::getUser();
		if ($categoryId)
		{
			// The category has been set. Check the category permissions.
			return $user->authorise('core.edit', 'com_phocagallery.phocagalleryc.'.$categoryId);
		}
		else
		{
