	...
	*/
	...
	public function uploadExtImagesFb($idCat, $data, &$errorMsg) {
		...
		if (!empty($user)) {
			...
			if(!empty($image)) {
				...
				$refreshUrl	= 'index.php?option=com_phocagallery&task=phocagalleryc.uploadextimgfpgn&id='.$idCat.'&fbalbum='.$data['extfbcatid'].'&fbuser='.$data['extfbuid'].'&fbimg='.(int)$image['id'] .'&amp;'. JUtility::getToken().'=1';
	...
	public function loadExtImagesFb($idCat, $data, &$errorMsg) {
		...
		if(count($dataImg) > 0) {
			...
			if($this->storeImage($dataImg, $data['id'], 'facebook')) {
				return true;
			} else {
	...
	public function loadExtImages($idCat, $data, &$message) {
		...
		if ($id && $id > 0) {
			...
			if ($album && (int)$album['id'] > 0) {
				...
				$picImg = $this->picasaImages($data['extu'],$data['extauth'], $album['id'], $id, $pagination, $errorMsgI);
				
				if (!$picImg) {
					$message = PhocaGalleryUtils::setErrorMsg($errorMsgI, $message);
					return false;
				} else {
					...
					if (isset($album['num']) && (int)$album['num'] > 0) {
						...
						if ((int)$loop > 50 || $maxCount < 20) {
							...
							if ((int)$album['num'] > (int)$newStartIf) {
								...
								$refreshUrl	= 'index.php?option=com_phocagallery&task=phocagalleryc.loadextimgpgn&id='.$id.'&picalbum='.$data['exta'].'&picuser='.$data['extu'].'&picauth='.$data['extauth'].'&picstart='.(int)$newStart .'&amp;'. JUtility::getToken().'=1';
