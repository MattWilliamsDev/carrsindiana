...
$canOrder	= $user->authorise('core.edit.state', 'com_phocagallery.phocagallerycos');
$saveOrder	= 'a.ordering';
?>

<form action="<?php echo JRoute::_('index.php?option=com_phocagallery&view=phocagallerycos'); ?>" method="post" name="adminForm" id="adminForm">
	...
	<tr>
		...
		<th width="10%">
			...
			if ($canOrder && $saveOrder) {
				echo JHtml::_('grid.order',  $this->items, 'filesave.png', 'phocagallerycos.saveorder');
			} ?>
		</th>
...
$linkEdit	= JRoute::_( 'index.php?option=com_phocagallery&task=phocagalleryco.edit&id='.(int) $item->id );
$linkCat	= JRoute::_( 'index.php?option=com_phocagallery&task=phocagalleryc.edit&id='.(int) $item->category_id );
$canEditCat	= $user->authorise('core.edit', 'com_phocagallery.phocagalleryc.'.(int)$item->category_id);

$date		= JHTML::_('date',  $item->date, JText::_('DATE_FORMAT_LC2') );
$canCheckin	= $user->authorise('core.manage',		'com_checkin') || $item->checked_out==$user->get('id') || $item->checked_out==0;
$canChange	= $user->authorise('core.edit.state',	'com_phocagallery.phocagallerycos.'.$item->category_id) && $canCheckin;
$canCreate	= $user->authorise('core.create',		'com_phocagallery.phocagallerycos.'.$item->category_id);
?>
<tr class="row<?php echo $i % 2; ?>">
	...
	<?php if ($item->checked_out) {
		echo JHtml::_('jgrid.checkedout', $i, $item->editor, $item->checked_out_time, 'phocagallerycos.', $canCheckin); 
	} 
	...
	<td class="center"><?php echo JHtml::_('jgrid.published', $item->published, $i, 'phocagallerycos.', $canChange); ?></td>
	
	
	<?php
	$cntx = 'phocagallerycos';
echo '<td class="order">';
if ($canChange) {
	if ($saveOrder) {
		if ($listDirn == 'asc') {
