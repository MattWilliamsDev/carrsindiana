...
$canOrder	= $user->authorise('core.edit.state', 'com_phocagallery.phocagalleryfbs');
$saveOrder	= 'a.ordering';
?>

<form action="<?php echo JRoute::_('index.php?option=com_phocagallery&view=phocagalleryfbs'); ?>" method="post" name="adminForm" id="adminForm">
...
<thead>
	<tr>
		...
		<th class="title">
			<?php echo JHtml::_('grid.sort', 'COM_PHOCAGALLERY_NAME', 'a.name', $listDirn, $listOrder); ?>
		</th>
		
		<th class="title">
			<?php echo JHtml::_('grid.sort', 'COM_PHOCAGALLERY_FB_USER_ID', 'a.uid', $listDirn, $listOrder); ?>
		</th>
		<th class="title">
			<?php echo JHtml::_('grid.sort', 'COM_PHOCAGALLERY_FB_APP_ID', 'a.appid', $listDirn, $listOrder); ?>
		</th>
		...
		<th width="10%">
			...
			if ($canOrder && $saveOrder) {
				echo JHtml::_('grid.order',  $this->items, 'filesave.png', 'phocagalleryfbs.saveorder');
			} ?>
		</th>
...
$canChange	= $user->authorise('core.edit.state',	'com_phocagallery.phocagalleryfbs.'.$item->id) && $canCheckin;
$canCreate	= $user->authorise('core.create',		'com_phocagallery.phocagalleryfbs.'.$item->id);
?>
<tr class="row<?php echo $i % 2; ?>">
	...
	<td><?php if (isset($item->uid) && $item->uid!= '') {
		echo '<img src="https://graph.facebook.com/'. $item->uid .'/picture">';
	} ?>
	</td>
	
	<td>
	<?php if ($item->checked_out) {
		echo JHtml::_('jgrid.checkedout', $i, $item->editor, $item->checked_out_time, 'phocagalleryfbs.', $canCheckin); 
	}
	...
	if ($canCreate || $canEdit) {
		echo '<a href="'. JRoute::_($linkEdit).'">'. $this->escape($item->uid).'</a>';
	} else {
		echo $this->escape($item->uid);
	}  ?>
	...
	if ($canCreate || $canEdit) {
		echo '<a href="'. JRoute::_($linkEdit).'">'. $this->escape($item->appid).'</a>';
	} else {
		echo $this->escape($item->appid);
	}  ?>
	
	</td>
	
	
	<td class="center"><?php echo JHtml::_('jgrid.published', $item->published, $i, 'phocagalleryfbs.', $canChange); ?></td>
	
	
	<?php
	$cntx = 'phocagalleryfbs';
echo '<td class="order">';
if ($canChange) {
	if ($saveOrder) {
		if ($listDirn == 'asc') {
