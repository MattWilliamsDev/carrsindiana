<?php
/**
 * @version $Id: copy_action.php 147 2009-07-14 20:20:18Z  $
...
/* set the page titles */
$controller->setPageTitle ( bfText::_ ( 'Copy Form Actions' ) );
$controller->setPageHeader ( bfText::_ ( 'Copy Form Actions' ) );
...
echo '<h1>' . bfText::_ ( 'Actions to Copy' ) . ' </h1>';

echo '<ul style="margin-left:100px;">';
foreach ( $action ['actionnames'] as $name ) {
	echo '<li>' . $name . '</li>';
}
echo '</ul>';
...
echo '<input type="hidden" name="actions" id="actions" value="' . base64_encode ( $action ['idstocopy'] ) . '" />';
?>