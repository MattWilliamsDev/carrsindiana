<?php
/**
 * @version $Id: listmessenger.php 147 2009-07-14 20:20:18Z  $
 ...
 *
 */
class plugins_actions_listmessenger extends plugins_actions_base {
	 ...
	 */
	private $_title = 'Subscribe Submitters To ListMessenger Mailing Lists';
	 ...
	 * @var array The defaults
	 */
	public $_creation_defaults = array ('plugin' => 'listmessenger', 'published' => '1', 'access' => '0', 'form_id' => '-1', 'custom4' => 'POST', 'custom6' => '1', 'custom5' => '' );
	 ...
	 */
	private $_desc = "AutoSubscribe to ListMessenger Mailing Lists";
	...
	public function run($return = false) {
		...
		foreach ($this->submittedData as $data=>$vars){
			$dataArray[$data] = $this->submittedData[$data]['submission'];
		}
		...
		$dataArray ['option'] = 'com_mailinglist';
		$dataArray ['tmpl'] = 'component';
		$dataArray ['group_ids'] = $this->_config ['custom5'];
		
		if ($this->_config ['custom6'] == '1') {
			$dataArray ['action'] = 'subscribe';
		} else {
			$dataArray ['action'] = 'unsubscribe';
		}
		
		$httppost = new HTTPPost ( str_replace ( 'https', 'http', bfCompat::getLiveSite () ) . '/index.php', $dataArray );
		if ($httppost->post ()) {
			echo '<!-- POSTED TO LISTMESSENGER BEHIND SCENES -->';
		}
	}
	...
	public function _editActionView() {
		...
		$custom4 = bfHTML::selectList2 ( $OPTIONS, 'custom4', 'class="inputbox bfinputbox"', 'value', 'text', $this->_config ['custom4'] );
		$tmp->assign ( 'CUSTOM4', $custom4 );
		
		$custom5 = bfHTML::selectList2 ( $groups, 'custom5', 'class="inputbox bfinputbox"', 'value', 'text', $this->_config ['custom5'] );
