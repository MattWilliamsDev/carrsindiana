...
class modJTSlideShowHelper
{
    function getParameterList( &$params )
    {
		...
		// Slimbox 2
		$this->labelImage				= JText::_( 'LIGHTBOX-IMAGE');
		$this->labelOf					= JText::_( 'LIGHTBOX-OF');
		$this->nowrap					= trim ( $params->get ( 'nowrap', 0 ) );
		$this->slimboxloop				= trim ( $params->get ( 'slimboxloop', 1 ) );
		if ($this->slimboxloop == 1) 
			$this->slimboxloop = 'true';
		else
			$this->slimboxloop = 'false';
		$this->slimboxoverlayOpacity	= trim ( $params->get ( 'slimboxoverlayOpacity', 0.8 ) );
		$this->slimboxoverlayColor		= trim ( $params->get ( 'slimboxoverlayColor', '#000' ) );
		$this->slimboxoverlayFadeDuration = trim ( $params->get ( 'slimboxoverlayFadeDuration', 400 ) );
		$this->slimboxresizeDuration	= trim ( $params->get ( 'slimboxresizeDuration', 400 ) );
		$this->slimboxresizeEasing		= trim ( $params->get ( 'slimboxresizeEasing', 'jswing' ) );
		$this->slimboxresizeEasing 		= '"'. $this->slimboxresizeEasing .'"';
		$this->slimboxinitialWidth		= trim ( $params->get ( 'slimboxinitialWidth', 250 ) );
		$this->slimboxinitialHeight		= trim ( $params->get ( 'slimboxinitialHeight', 250 ) );
		$this->slimboximageFadeDuration	= trim ( $params->get ( 'slimboximageFadeDuration', 400 ) );
		$this->slimboxcaptionAnimationDuration = trim ( $params->get ( 'slimboxcaptionAnimationDuration', 400 ) );
		$this->slimboxcloseKeys			= trim ( $params->get ( 'slimboxcloseKeys',  '27, 88, 67' ) );
		$this->slimboxcloseKeys 		= '['. $this->slimboxcloseKeys .']';
		$this->slimboxpreviousKeys		= trim ( $params->get ( 'slimboxpreviousKeys', '37, 80' ) );
		$this->slimboxpreviousKeys 		= '['. $this->slimboxpreviousKeys .']';
		$this->slimboxnextKeys			= trim ( $params->get ( 'slimboxnextKeys',  '39, 78' ) );
		$this->slimboxnextKeys 			= '['. $this->slimboxnextKeys .']';
		...
		$this->showmetatag				= $params->get( 'showmetatag', 1 );
		...
		// Slimbox 2 Single image or image set
		$this->Slimboxset = '';
		if (count($this->image) > 1)
			$this->Slimboxset = ' rel="lightbox-'.$this->boxname.'"';
		else 
			$this->Slimboxset = ' rel="lightbox"';
		...
		for ( $i=0 ; $i < count($this->image) ; $i++ ) {
			...
			// Show Metatags
			if ($this->showmetatag) {
				// If number of titles are equal or smaller than number of images
			...
			// Preparing Links
			if ($this->linked == 1)
				$this->imagelinkstart[$i] 	= '<a href="'. $this->url[$i] .'" target="'. $this->target .'">';
			else if ($this->linked == 2)
				$this->imagelinkstart[$i] 	= '<a href="'.JURI::root() . $this->folder . '/' . $this->image[$i].'" '. $this->alttitle[$i] . $this->Slimboxset . ' >';
			else 
				$this->imagelinkstart[$i] 	= '<a href="javascript:void(0);">';
			$this->imagelinkend		 	= '</a>';
		...
		if ($this->linked == 2) {
		// Slimbox 2 Style
			$Slimboxcss = ' #lbOverlay { position: fixed; z-index: 9999; left: 0; top: 0; width: 100%; height: 100%; background-color: '. $this->slimboxoverlayColor .'; cursor: pointer; } ';
			$Slimboxcss = $Slimboxcss . ' #lbCenter, #lbBottomContainer { position: absolute; z-index: 9999; overflow: hidden; background-color: #fff; } ';
			$Slimboxcss = $Slimboxcss . ' .lbLoading { background: #fff url(' . JURI::root() . 'modules/mod_jt_slideshow/images/loading.gif) no-repeat center; } ';
			$Slimboxcss = $Slimboxcss . ' #lbImage { position: absolute; left: 0; top: 0; border: 10px solid #fff; background-repeat: no-repeat; } ';
			$Slimboxcss = $Slimboxcss . ' #lbPrevLink, #lbNextLink { display: block; position: absolute; top: 0; width: 50%; outline: none; } ';
			$Slimboxcss = $Slimboxcss . ' #lbPrevLink { left: 0; } ';
			$Slimboxcss = $Slimboxcss . ' #lbPrevLink:hover { background: transparent url(' . JURI::root() . 'modules/mod_jt_slideshow/images/prevlabel.gif) no-repeat 0 15%; } ';
			$Slimboxcss = $Slimboxcss . ' #lbNextLink { right: 0; } ';
			$Slimboxcss = $Slimboxcss . ' #lbNextLink:hover { background: transparent url(' . JURI::root() . 'modules/mod_jt_slideshow/images/nextlabel.gif) no-repeat 100% 15%; } ';
			$Slimboxcss = $Slimboxcss . ' #lbBottom { font-family: Verdana, Arial, Geneva, Helvetica, sans-serif; font-size: 10px; color: #666; line-height: 1.4em; text-align: left; border: 10px solid #fff; border-top-style: none; } ';
			$Slimboxcss = $Slimboxcss . ' #lbCloseLink { display: block; float: right; width: 66px; height: 22px; background: transparent url(' . JURI::root() . 'modules/mod_jt_slideshow/images/closelabel.gif) no-repeat center; margin: 5px 0; outline: none; } ';
			$Slimboxcss = $Slimboxcss . ' #lbCaption, #lbNumber { margin-right: 71px; } ';
			$Slimboxcss = $Slimboxcss . ' #lbCaption { font-weight: bold; } ';
		}
		...
		// loading Lightbox 2 Scripts
		if ($this->linked == 2)
			$Slimbox = '<script src = "'. JURI::root() .'modules/mod_jt_slideshow/scripts/slimbox2.js" type="text/javascript"></script>' . "\n";
		
		// Apply CSS Styles & JavaScript
		$document =& JFactory::getDocument();
		$document->addCustomTag($jQuery);
		if ($this->linked == 2)
			$document->addCustomTag($Slimbox);
		$document->addStyleDeclaration($css);
		if ($this->linked == 2)
			$document->addStyleDeclaration($Slimboxcss);
