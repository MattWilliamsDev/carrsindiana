	...
	initialize: function(el, data, options){
		...
		if (!data){
			...
			this.slideshow.getElements(this.classes.get('images') + ' img').each(function(img, i){
				var src = img.get('src');
				var caption = [img.get('alt'), img.get('title'), ''].pick();
		...
		this.events = {'keydown': [], 'keyup': [], 'mousemove': []};
...
*/
	...
	destroy: function(p){
		...
		this.pause(1);
		if (this.options.loader)
			window.clearTimeout(this.slideshow.retrieve('loader').retrieve('timer'));
		if (this.options.thumbnails)
			window.clearTimeout(this.slideshow.retrieve('thumbnails').retrieve('timer'));
		this.slideshow.uid = Date.now();  // Native.UID++;
		if (p)
			this.slideshow[p]();
	},
...
*/
	...
	_preload: function(fast){
		fast = undefined;  // fix by Levente Hunyadi to avoid abrupt changes instead of smooth transition in slideshow
		if (!this.preloader)
...
*/
	...
	_controller: function(){
		...
		Object.each({'first': 'Shift + Leftwards Arrow', 'prev': 'Leftwards Arrow', 'pause': 'P', 'next': 'Rightwards Arrow', 'last': 'Shift + Rightwards Arrow'}, function(accesskey, action){
...
*/
	...
	_loader: function(){
		...
		var loader = new Element('div', {
			...
			'morph': Object.merge({}, this.options.loader, {'link': 'cancel'})
		...
		loader.set('events', {
			...
			'hide': function(){
				...
				if (!loader.retrieve('hidden')){
					...
					if (this.options.loader.animate)
						window.clearInterval(loader.retrieve('timer'));
				}
			}.bind(this),
			'show': function(){
				var loader = this.slideshow.retrieve('loader');
		...
		var mousemove = function(e){
			...
			if (e.page.x > div.left && e.page.x < div.right && e.page.y > div.top && e.page.y < div.bottom){
				...
				if (this.retrieve('mouseover')){
					...
					window.clearInterval(this.retrieve('timer'));
				}
			}
		}.bind(thumbnails);
		this.events.mousemove.push(mousemove);
