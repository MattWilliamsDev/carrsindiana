...
<table class="bfadminlist">
	...
	<tr>
		...
		{bfText}This is the public title and IS SHOWN on the site{/bfText}
		</td>
		<td><input size="100" class="inputbox bfinputbox" type="text"
		...
		<td><input onblur="bf_form_admin.validateFieldSlug();" size="100" class="inputbox bfinputbox" type="text"
			value="{$SLUG}" name="slug" id="slug" /><br />
		<b>{bfText}This must be unique in this form, this means you cannot have two elements with the same system name!!!<br />You should also keep this quite short, for example: "lastname"{/bfText}</b></td>
	</tr>
	<tr>
...
<div id="fragment-2">
     <table class="bfadminlist">
     <tr>
     		<td valign="top"><b>{bfText}Drop Down Values{/bfText}</b> <br />
     		{bfText}Enter the options for the drop down here.  This should be in one of two formats:{/bfText}
     		<br />
     		{bfText}1)  If you want to set a value to be saved different to the visible<br /> text then options should be formated one per line like this:{/bfText}
     		<br >
     		VALUE|TEXT<br />
     		0|No<br />
     		1|Yes<br />
     		m|Maybe<br />
     		<br />
		{bfText}2)  If you want to save the value of the visable text then options<br /> should be formated one per line like this:{/bfText}
		<br >
     		VALUE<br />
     		No<br />
     		Yes<br />
     		...
     		<td valign="top">
     		<textarea id="params" name="params" class="inputbox bfinputbox" style="width: 400px; height: 100px;">{$PARAMS}</textarea></td>
     	</tr>
     	<tr>
     	...
     	<tr>
     		<td><b>{bfText}Allow population of the value pairs by SQL Query{/bfText}</b> <br />
     		{bfText}If this is enabled AND the "Drop Down Values" above is left blank, then we will run the sql below to collect the value pairs{/bfText}</td>
     		<td>{$OPTION2}</td>
     	</tr>
     	
     	 <tr>
     		<td valign="top"><b>{bfText}SQL Query to load value pairs from database{/bfText}</b> <br />
     		{bfText}If you enabled "Allow population of the value pairs by SQL Query" above, then you need to enter a SQL query in here to get the value pairs, the SQL should return value and text, here is an example:{/bfText}
     		<br />
     		SELECT id as value, title as text from jos_content;
     		<br />
     		SELECT username as value, username as text from jos_users;
     		<br >
     		</td>
     		<td valign="top">
     		<textarea id="populatebysql" name="populatebysql" class="inputbox bfinputbox" style="width: 400px; height: 100px;">{$POPULATEBYSQL}</textarea></td>
     	</tr>
     	
     	
     	
     	<tr>
     		<td><b>{bfText}Allow Multiple Selections{/bfText}</b> <br />
     		</td><td>{$MULTIPLE}</td>
     	</tr>
     	<tr>
     		<td><b>{bfText}Allow default value to be overridden by var in the GET{/bfText}</b> <br />
     		{bfText}If this is set to YES, then calling a form with a url containing &FIELDNAME=myvalue would set the default value to myvalue{/bfText}</td>
     		<td>{$ALLOWSETBYGET}</td>
     	</tr>
     	<tr>
     	...
     	<tr>
     		...
     		{bfText}Enter the Javascript onBlur statement here, the application will insert this in the onBlur="" attribute{/bfText}</td>
     		<td>
     		<textarea id="onblur" name="onblur" class="inputbox bfinputbox" style="width: 400px; height: 100px;">{$ONBLUR}</textarea></td>
     	</tr>
     </table>

</div>
<div id="fragment-2a">
	<h1>{bfText}Warning: Pay attention to the options here, setting all to yes will make it impossible for this field to pass validation, some rules will conflict with other rules! (i.e. input cannot be expected to be an IP Address AND an email address!!!{/bfText}</h1>
...
<table class="bfadminlist">
	...
	<tr>
		<td><b>{bfText}Input MUST be a valid Bank IBAN Number{/bfText}</b> <br />
		{bfText}Setting this to "Yes" will force this field to only accept
		valid{/bfText} <a href="http://en.wikipedia.org/wiki/International_Bank_Account_Number">bank IBAN Numbers</a></td>
		<td>{$VERIFY_IBAN}</td>
	</tr>
	</table>
	</fieldset>
...
<div id="fragment-2b">
     <table class="bfadminlist">
     	<tr>
     		...
     		{bfText}Setting this to "Yes" will force this field to be a filtered to only a-z or A-Z chars{/bfText}</td>
     		<td>{$FILTER_A2Z} <br /><b>{bfText}Default: No{/bfText}<b></td>
     	</tr>
     	<tr>
     		<td><b>{bfText}StripTags Filter{/bfText}</b> <br />
     		{bfText}Setting this to "Yes" will force this field to be a filtered removing all HTML Tags{/bfText}</td>
     		<td>{$FILTER_STRIPTAGS} <br /><b>{bfText}Default: Yes{/bfText}<b></td>
     	</tr>
     	<tr>
     		<td><b>{bfText}Trim Input{/bfText}</b> <br />
     		{bfText}Setting this to "Yes" will trim the preceeding and trailing whitespace from the submitted value{/bfText}
     		<td>{$FILTER_STRINGTRIM} <br /><b>{bfText}Default: Yes{/bfText}<b></td>
     	</tr>
     	<tr>
     		<td><b>{bfText}Alnum Filter{/bfText}</b> <br />
     		{bfText}Setting this to "Yes" will returns the value, removing all but alphabetic and digit characters.{/bfText}
     		<td>{$FILTER_ALNUM} <br /><b>{bfText}Default: No{/bfText}<b></td>
     	</tr>
     	<tr>
     		<td><b>{bfText}Digits Only{/bfText}</b> <br />
     		{bfText}Setting this to "Yes" will returns the value, removing all but digit characters.{/bfText}
     		<td>{$FILTER_DIGITS} <br /><b>{bfText}Default: No{/bfText}<b></td>
     	</tr>
     	<tr>
...
<table class="bfadminlist">
     	<tr>
     		...
     		{bfText}A visitor needs to be this level or below to see this field{/bfText}</td>
     		<td>{$ACCESS} <br /><b>{bfText}Default: Public{/bfText}<b></td>
     	</tr>
     	<tr>
     		<td><b>{bfText}Publish this field{/bfText}</b> <br />
     		{bfText}Toggle this field published/unpublished{/bfText}</td>
     		<td>{$PUBLISHED} <br /><b>{bfText}Default: Yes{/bfText}<b></td>
     	</tr>
     	<tr>
     		<td><b>{bfText}Allow submitted value to be sent by email{/bfText}</b> <br />
     		{bfText}If this field contains confidential text and you never want this to be sent by email then choose No, and we will only send *****'s by email instead{/bfText}</td>
     		<td>{$ALLOWBYEMAIL} <br /><b>{bfText}Default: Yes{/bfText}<b></td>
     	</tr>
...
<div id="fragment-4"><!--  Style -->
 <table class="bfadminlist">
     	...
     	<tr>
     		...
     		{bfText}If you put css in the text box to the right, this will be put in a style="" attribute in the HTML tag{/bfText}</td>
     		<td><input size="100" class="inputbox bfinputbox" type="text"
     			value="{$STYLE}" name="style" id="style" /></td>
     	</tr>
     	<tr>
     		<td><b>{bfText}Field Size{/bfText}</b> <br />
     		{bfText}The size attribute of the field's element, this dictates the number of lines viewable.  Default: 1{/bfText}</td>
     		<td><input size="4" class="inputbox bfinputbox" type="text"
     			value="{$SIZE}" name="size" id="size" /></td>
     	</tr>
     	<tr>
     		<td><b>{bfText}Enable alternative multiple selection process{/bfText}</b> <br />
     		{bfText}If this element has MULTIPLE selected then this option turns on some really nice effects!{/bfText}</td>
     		<td>{$OPTION1}</td>
     	</tr>
     </table>
</div>
